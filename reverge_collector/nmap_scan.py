...
class Nmap(ToolSpec):
        ...
        if json_input:
            ...
            for entry in nmap_scan_obj.get('nmap_scan_list', []):
                ...
                if os.path.exists(nmap_out) and os.path.getsize(nmap_out) > 0:
                    try:
                        ret_arr.extend(
                            parse_nmap_xml(nmap_out, scope_obj, tool_instance_id, tool_id)
                        )
                    except Exception:
    ...
    def nmap_modules() -> List:
        ...
        return get_cached_modules('nmap', Nmap._fingerprint, Nmap._generate_nmap_modules)
    ...
    def _generate_nmap_modules() -> List:
        ...
        try:
            ...
            while i < len(lines):
                ...
                if not line.startswith('\t') and not line.startswith(' '):
                    ...
                    if i < len(lines) and lines[i].startswith('Categories:'):
                        categories = lines[i].replace('Categories:', '').strip()
                        i += 1
                    else:
                        categories = ''
            ...
            logging.getLogger(__name__).error(f'Error getting nmap modules: {str(e)}')
...
def execute_scan(scan_input) -> None:
    ...
    for scan_obj in nmap_scan_list:
        ...
        ip_list: Set[str] | List[str] = scan_obj['ip_set']
        if len(ip_list) == 0:
            continue
        with open(ip_list_path, 'w') as in_file_fd:
            for ip in ip_list:
                in_file_fd.write(ip + '\n')
        nmap_output_xml_file: str = dir_path + os.path.sep + 'nmap_out_' + str(counter)
        command: List[str] = []
        if os.name != 'nt':
            command.append('sudo')
        ...
        for future in futures:
            ...
            if ret_dict and 'exit_code' in ret_dict:
                ...
                if exit_code != 0:
                    ...
                    logging.getLogger(__name__).error(
                        'Nmap scan for scan ID %s exited with code %d: %s'
    ...
    for host in nmap_report.hosts:
        ...
        for port_tuple in host.get_open_ports():
            ...
            for script in script_res:
                ...
                if script_id == 'ssl-cert':
                    ...
                    if 'notBefore' in validity:
                        try:
                            dt = datetime.strptime(validity['notBefore'], '%Y-%m-%dT%H:%M:%S')
                    ...
                    if 'notAfter' in validity:
                        try:
                            dt = datetime.strptime(validity['notAfter'], '%Y-%m-%dT%H:%M:%S')
            ...
            for script_out in script_res:
                ...
                module_output_obj = data_model.CollectionModuleOutput(parent_id=temp_module_id)
