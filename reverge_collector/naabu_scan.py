        ...
        if json_input:
            ...
            for entry in naabu_scan_obj.get('naabu_scan_list', []):
                ...
                if os.path.exists(naabu_out) and os.path.getsize(naabu_out) > 0:
                    try:
                        ret_arr.extend(parse_naabu_output(naabu_out, scope_obj, tool_instance_id))
                    except Exception:
...
def execute_scan(scan_input) -> None:
    ...
    if os.path.exists(meta_file_path):
        logging.getLogger(__name__).debug(
            'Output path %s already exists, skipping naabu scan execution', meta_file_path
        )
        return
    ...
    for scan_obj in naabu_scan_list:
        ...
        naabu_output_file = dir_path + os.path.sep + 'naabu_out_' + str(counter)
        ...
        futures.append(
            scan_utils.executor.submit(process_wrapper, cmd_args=command, pid_callback=callback)
        )
        counter += 1
    ...
    with open(output_path) as fh:
        for raw_line in fh:
            ...
            try:
                ...
                logging.getLogger(__name__).warning('Skipping invalid IP address: %s', ip)
                continue
