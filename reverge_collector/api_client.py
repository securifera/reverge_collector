...
class ApiClient:
    ...
    def __init__(self, token: str, manager_url: str) -> None:
        ...
        self.verify_ssl: bool = os.environ.get('REVERGE_VERIFY_SSL', 'true').lower() in (
            '1',
            'true',
            'yes',
        )
    ...
    def _init_session_key(self) -> bytes:
        ...
        try:
            ...
            raise RuntimeError('Reverge session key exchange failed: %s' % exc) from exc
    ...
    def _post(
        self, path: str, payload: Any, *, expect_response: bool = False, as_namespace: bool = False
    ) -> Any:
        """
        ...
        * HTTP 5xx — the server was temporarily unavailable; the server is
        ...
        for attempt in range(_POST_MAX_RETRIES + 1):
            ...
            if r.status_code >= 500:
                ...
                last_exc = RuntimeError('POST %s returned HTTP %d' % (path, r.status_code))
                continue
            if r.status_code != 200:
        ...
        if last_exc:
            raise last_exc
        raise RuntimeError('POST %s failed after %d attempts' % (path, _POST_MAX_RETRIES + 1))
    ...
    def update_scan_status(
        ...
        result = self._post('/api/scheduler/%s/' % schedule_scan_id, payload)
        if result is None:
            from reverge_collector.recon_manager import ScanNotFoundException

            raise ScanNotFoundException('Scan %s not found on server' % schedule_scan_id)
        return True
    ...
    def import_shodan_data(self, scan_id: str, shodan_arr: List[Any]) -> bool:
        ...
        self._post('/api/integration/shodan/import/%s' % str(scan_id), shodan_arr)
        return True
