...
_OPT_RHOSTS = {'type': 'rhosts', 'required': True, 'advanced': False, 'desc': 'Target host(s)'}
_OPT_RPORT_445 = {
    'type': 'port',
    'required': True,
    'advanced': False,
...
_OPT_DOMAIN = {'type': 'string', 'required': True, 'advanced': False, 'desc': 'Target domain'}
...
class TestMetasploitScan:
    ...
    def _make_scan_objects(
        self, recon_manager, port_list='135, 445', args='auxiliary/scanner/smb/smb_ms17_010'
    ):
        ...
        sched_scan_arr = json.loads(
            json.dumps(scheduler_inst_object), object_hook=lambda d: SimpleNamespace(**d)
        )
    ...
    def test_metasploit_scan_success(self, recon_manager):
        ...
        with (
            patch.object(ReconManager, 'get_scheduled_scan', return_value=scan_data),
            patch(
                'reverge_collector.metasploit_scan.execute_msfrpc_commands',
            ...
            input_conf = '%s/%s-outputs/%s_in_0' % (output_dir, self.TOOL_NAME, self.TOOL_NAME)
            assert os.path.exists(input_conf) == True
            output_file = '%s/%s-outputs/%s_out_0' % (output_dir, self.TOOL_NAME, self.TOOL_NAME)
    ...
    def _mock_post(self, url, json=None, **kwargs):
        ...
        if method == 'module.search':
            ...
            if info:
                mock_resp.json.return_value = {'jsonrpc': '2.0', 'result': info}
            else:
                mock_resp.json.return_value = {'jsonrpc': '2.0', 'result': {}}
    ...
    def test_generate_modules_descriptions_populated(self):
        ...
        with patch('reverge_collector.metasploit_scan.requests.post', side_effect=self._mock_post):
            modules = Metasploit._generate_metasploit_modules('127.0.0.1', 8081, 'token', False)
        ...
        assert (
            smb.description
            == MSF_INFO_RESPONSES[('auxiliary', 'scanner/smb/smb_ms17_010')]['description']
        )
        ...
        assert (
            eternal.description
            == MSF_INFO_RESPONSES[('exploit', 'windows/smb/ms17_010_eternalblue')]['description']
        )
    ...
    def test_generate_modules_missing_description_falls_back_to_fullname(self):
        ...
        with patch('reverge_collector.metasploit_scan.requests.post', side_effect=_mock_no_desc):
            modules = Metasploit._generate_metasploit_modules('127.0.0.1', 8081, 'token', False)
