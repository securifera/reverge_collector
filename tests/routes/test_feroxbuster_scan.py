...
def test_feroxbuster_scan_success(recon_manager):

    tool_id = None
    tool_map = recon_manager.get_tool_map()
    for tool_id_tmp, tool in tool_map.items():
        if tool.name == 'feroxbuster':
            tool_id = tool_id_tmp
            break

    assert tool_id is not None, "Tool not found in tool map"
    ...
    tool_inst = {'id': 'a9866b94f7104754bd161c1ab7cbf0cd', 'collection_tool': {'wordlists': [wordlist_inst], 'name': 'feroxbuster', 'args':
                                                                               '--rate-limit 50 -s 200 -n', 'tool_type': 2, 'scan_order': 2, 'api_key': None, 'id': tool_id}, 'args_override': None,
                 'enabled': 1, 'status': 0, 'status_message': None, 'collection_tool_id': tool_id,
    ...
    with patch.object(ReconManager, 'get_scheduled_scan', return_value=scan_data):
        with patch.object(ReconManager, 'get_wordlist', return_value=wordlist_data):
            ...
            target_output = "%s/feroxbuster-outputs/ferox_outputs_%s" % (
...
def test_feroxbuster_import_success(recon_manager):
        ...
        with patch.object(ReconManager, 'get_scheduled_scan', return_value=scan_data):
            with patch.object(ReconManager, 'get_wordlist', return_value=wordlist_data):
                ...
                with patch.object(ReconManager, 'import_data', return_value={}):
                    ...
                    assert result == True
                    output_json = "%s/feroxbuster-outputs/tool_import_json" % (
                        output_dir)
