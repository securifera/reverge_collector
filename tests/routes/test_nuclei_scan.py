    ...
    def test_nuclei_scan_success(self, recon_manager):
        ...
        tool_inst = {
            ...
            'collection_tool': {
                'wordlists': [],
                'name': self.TOOL_NAME,
                'args': '-ni -t http/technologies/fingerprinthub-web-fingerprints.yaml',
                'tool_type': 2,
                'scan_order': 2,
                'api_key': None,
                'id': tool_id_instance,
        ...
        with patch.object(ReconManager, 'get_scheduled_scan', return_value=scan_data):
            ...
            input_conf = '%s/%s-outputs/%s_scan_in' % (output_dir, self.TOOL_NAME, self.TOOL_NAME)
        ...
        try:
            ...
            with patch.object(ReconManager, 'get_scheduled_scan', return_value=scan_data):
                ...
                with patch.object(ReconManager, 'import_data', return_value={}):
                    ...
                    assert result == True
                    output_json = '%s/nuclei-outputs/tool_import_json' % (output_dir)
