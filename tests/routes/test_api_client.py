...
class TestApiClientInit:
    ...
    def test_init_session_key_failure_raises(self):
        with patch.object(
            ApiClient, '_init_session_key', side_effect=RuntimeError('handshake failed')
        ):
...
class TestDecrypt:
    ...
    def test_decrypt_refreshes_key_on_failure(self):
        ...
        with (
            patch('reverge_collector.tool_utils.decrypt_data', side_effect=fake_decrypt),
            patch.object(
                self.client, '_refresh_session_key', side_effect=fake_refresh
            ) as mock_refresh,
        ):
            result = self.client._decrypt({'data': 'b64stuff'})
    ...
    def test_decrypt_returns_none_when_all_attempts_fail(self):
        with (
            patch('reverge_collector.tool_utils.decrypt_data', side_effect=Exception('bad')),
